...
def generate_teams(num_teams):
    ...
    confed_weights = {
        confed: int(round((count / total_confed_slots) * num_teams))
        for confed, count in CONFED_COUNTS.items()
    }

    # Fix rounding issues to ensure exact num_teams
    while sum(confed_weights.values()) < num_teams:
        max_confed = max(confed_weights, key=lambda c: CONFED_COUNTS[c])
        confed_weights[max_confed] += 1
    while sum(confed_weights.values()) > num_teams:
        max_confed = max(confed_weights, key=lambda c: confed_weights[c])
        if confed_weights[max_confed] > 0:
            confed_weights[max_confed] -= 1

    # Build confed pool with adjusted weights
    confed_pool = sum([[c] * n for c, n in confed_weights.items()], [])
...
def parse_clingo_result(content):
    """Adapted from result_parser.py for Clingo results"""
    data = {
        'solver': 'Clingo',
        'status': 'UNKNOWN',
        'time': None,
    ...
    # Pattern to find CPU Time
    cpu_match = re.search(r"CPU Time\s*:\s*([\d.]+)s", content)
    ...
    if cpu_match:
        data['cpu_time'] = float(cpu_match.group(1))
...
def parse_cpsat_result(content):
    """Adapted from result_parser.py for CP-SAT results"""
    data = {
        'solver': 'CP-SAT',
        'status': 'UNKNOWN',
        'time': None,
...
st.title("Sports Timetabling with Answer Set Programming")
...
with col2:
    st.text("Custom Facts (Override generated facts)")
    
    custom_teams = st.text_input(
        "Custom Teams (comma-separated)", 
        placeholder="brazil, germany, argentina, france",
        help="Leave empty to use default t1, t2, etc."
    )
    
    custom_venues = st.text_input(
        "Custom Venues (comma-separated)",
        placeholder="wembley, maracana, allianz_arena",
    ...
    if num_teams % 4 != 0:
        ...
        try:
            ...
            if custom_host.strip():
                ...
                host_facts = [f"host({team_names[0]})."]
...
with comparison_col1:
    ...
    if input1_method == "Paste":
        solver1_output = st.text_area(
            f"{solver1_type} Output",
            height=200,
...
with comparison_col2:
    ...
    if input2_method == "Paste":
        solver2_output = st.text_area(
            f"{solver2_type} Output",
            height=200,
...
if st.button("Compare Results", type="primary"):
    if not solver1_output and not solver2_output:
        ...
        if solver1_output:
            if solver1_type == "Clingo":
                solver1_data = parse_clingo_result(solver1_output)
            else:
                solver1_data = parse_cpsat_result(solver1_output)
        ...
        if comparison_data:
            ...
            available_metrics = [m for m in metrics_to_show if m in df.columns and not df[m].isna().all()]
            
            if available_metrics:
                st.dataframe(df[available_metrics], use_container_width=True)
           
st.divider()

# ==== PARSE AND DISPLAY SCHEDULE =====
st.subheader("Schedule Visualisation")

schedule_facts = ""

schedule_facts = st.text_area(
    "Paste an output schedule facts here:",
    height=300,
...
if st.button("Display Formatted Schedule") and schedule_facts:
    try:
        ...
        groups_data = [
            {"Team": g[0], "Group": g[1], "Position": int(g[2])}
            for g in group_pattern.findall(schedule_facts)
        ]
        ...
        if not matches_data and not groups_data:
            ...
            # Display tabs
            tab_overview, tab_daily, tab_groups = st.tabs([
                "📊 Overview", "📅 Daily Schedule", "🏆 Groups"
            ])
            ...
            with tab_overview:
                if not matches_df.empty:
                    ...
                    with col2:
                        st.metric("Tournament Days", matches_df['Day'].max())
                    with col3:
                        st.metric("Groups", matches_df['Group'].nunique())
                    with col4:
                        st.metric("Venues Used", matches_df['Venue'].nunique())
